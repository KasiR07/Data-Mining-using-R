...
#plot our kmeans clusters with different combination of 2 variables - 4 clusters
#to show the centriods you have to plot against 2 variables only
kmeans.result4 <- kmeans(data, 4) #4 clusters
kmeans.result4

#plot our kmeans clusters with different combination of 2 variables - 4 clusters
#to show the centriods you have to plot against 2 variables only
plot(data[c("Sepal.Length", "Sepal.Width")], col = kmeans.result4$cluster)
points(kmeans.result4$centers[,c("Sepal.Length", "Sepal.Width")], col = 1:3,  pch = 8, cex=2)

plot(data[c("Petal.Length", "Petal.Width")], col = kmeans.result4$cluster)
points(kmeans.result4$centers[,c("Petal.Length", "Petal.Width")], col = 1:3,  pch = 8, cex=2)

#to plot all variables together without trying different combination
plot(data, col = kmeans.result4$cluster)
points(kmeans.result4$centers, col = 1:3,  pch = 8, cex=2)
